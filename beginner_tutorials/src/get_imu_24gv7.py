...
if __name__ == '__main__':
    ...
    imu_pub2 = rospy.Publisher("/mag_raw", MagneticField, queue_size=10)
...
############################################
            ...
            magnet.header.seq=0
            magnet.header.stamp=rospy.Time.now()
            magnet.header.frame_id="imu_link"
            magnet.magnetic_field.x=round(float(data[6]),3)
            magnet.magnetic_field.y=round(float(data[7]),3)
            magnet.magnetic_field.z=round(float(data[8]),3)
            
            imu_pub.publish(imu)
            imu_pub2.publish(magnet)
